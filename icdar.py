...
tf.app.flags.DEFINE_integer('max_text_size', 800,
                            'if the text in t he input image is bigger than this, then we resize'
...
def generate_rbox(im_size, polys, tags):
    ...
    for poly_idx, poly_tag in enumerate(zip(polys, tags)):
        poly = poly_tag[0]
        print("poly is: "+str(poly))if LOG else None
        tag = poly_tag[1]
...
def generator(input_size=512, batch_size=32,
              ...
              vis=False):
    image_list = np.array(get_images())          # image_list: 所有图片
    ...
    while True:
        ...
        for i in index:
            try:
                im_fn = image_list[i]           # im_fn: 一张图片
                im = cv2.imread(im_fn)
                # print im_fn
                h, w, _ = im.shape
                txt_fn = im_fn.replace(os.path.basename(im_fn).split('.')[1], 'txt')     # txt_fn: 一张图片的标签文件
                if not os.path.exists(txt_fn):
                    print('text file {} does not exists'.format(txt_fn))
                    continue

                text_polys, text_tags = load_annoataion(txt_fn)         # text_polys：一张图片的多个polys
                print("text_polys: "+str(text_polys))if LOG else None
                ...
                text_polys *= rd_scale
                print("text_polys after random scale : "+str(text_polys))if LOG else None

                # print rd_scale
                # random crop a area from image
                if np.random.rand() < background_ratio:
                    print("background now")if LOG else None
                    # crop background
                    ...
                    print("no background")if LOG else None
                    im, text_polys, text_tags = crop_area(im, text_polys, text_tags, crop_background=False)
                    print("text_polys after random crop : " + str(text_polys))if LOG else None
                    ...
                    print("text_polys after pad and resize: " + str(text_polys))if LOG else None

                    new_h, new_w, _ = im.shape
                    print("ready to generate_rbox")if LOG else None
